...
class TestMxEdiCommon(AccountEdiTestCommon):
    ...
    @classmethod
    def setUpClass(cls, chart_template_ref='l10n_mx.mx_coa', edi_format_ref='mx_pac_edi.edi_cfdi_3_3'):
        ...
        cls.certificate = cls.env['mx_pac_edi.certificate'].create({
            'content': base64.encodebytes(misc.file_open(os.path.join('mx_pac_edi', 'demo', 'pac_credentials', 'certificate.cer'), 'rb').read()),
            'key': base64.encodebytes(misc.file_open(os.path.join('mx_pac_edi', 'demo', 'pac_credentials', 'certificate.key'), 'rb').read()),
            'password': '12345678a',
        })
        ...
        cls.company_data['company'].write({
            ...
            'mx_pac_edi_pac': 'solfact',
            'mx_pac_edi_pac_test_env': True,
            'mx_pac_edi_fiscal_regime': '601',
            'mx_pac_edi_certificate_ids': [(6, 0, cls.certificate.ids)],
        })

        cls.currency_data['currency'].mx_pac_edi_decimal_places = 3
        ...
        # by a CRON.
        xsd_attachment = cls.env.ref('mx_pac_edi.xsd_cached_cfdv33_xsd', False)
        if xsd_attachment:
