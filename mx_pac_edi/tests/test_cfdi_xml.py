...
class TestEdiResults(TestMxEdiCommon):
    ...
    def test_invoice_cfdi_addenda(self):
        with freeze_time(self.frozen_today):
            ...
            self.invoice.partner_id.mx_pac_edi_addenda = self.env['ir.ui.view'].create({
                'name': 'test_invoice_cfdi_addenda',
                'type': 'qweb',
                'arch': """
                    <t t-name="mx_pac_edi.test_invoice_cfdi_addenda">
    ...
    def test_payment_cfdi(self):
        with freeze_time(self.frozen_today):
            self.payment.payment_id.action_mx_pac_edi_force_generate_cfdi()
            self.invoice.action_post()
            ...
            self.invoice.mx_pac_edi_cfdi_uuid = '123456789'
