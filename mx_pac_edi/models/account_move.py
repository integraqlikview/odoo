...
CFDI_XSLT_CADENA = 'mx_pac_edi/data/3.3/cadenaoriginal.xslt'
CFDI_XSLT_CADENA_TFD = 'mx_pac_edi/data/xslt/3.3/cadenaoriginal_TFD_1_1.xslt'
...
class AccountMove(models.Model):
    ...
    # ==== CFDI flow fields ====
    mx_pac_edi_cfdi_request = fields.Selection(
        selection=[
        ...
        compute='_compute_mx_pac_edi_cfdi_request',
        help="Flag indicating a CFDI should be generated for this journal entry.")
    mx_pac_edi_sat_status = fields.Selection(
        selection=[
    ...
    mx_pac_edi_certificate_id = fields.Many2one(
        comodel_name='mx_pac_edi.certificate',
        string="Source Certificate")
    mx_pac_edi_cer_source = fields.Char(
    ...
    mx_pac_edi_cfdi_uuid = fields.Char(string='Fiscal Folio', copy=False, readonly=True,
        help='Folio in electronic invoice, is returned by SAT when send to stamp.',
        compute='_compute_cfdi_values')
    mx_pac_edi_cfdi_supplier_rfc = fields.Char(string='Supplier RFC', copy=False, readonly=True,
        help='The supplier tax identification number.',
        compute='_compute_cfdi_values')
    mx_pac_edi_cfdi_customer_rfc = fields.Char(string='Customer RFC', copy=False, readonly=True,
        help='The customer tax identification number.',
        compute='_compute_cfdi_values')
    mx_pac_edi_cfdi_amount = fields.Monetary(string='Total Amount', copy=False, readonly=True,
        help='The total amount reported on the cfdi.',
        compute='_compute_cfdi_values')

    # ==== Other fields ====
    mx_pac_edi_payment_method_id = fields.Many2one('mx_pac_edi.payment.method',
        string="Payment Way",
        ...
        default=lambda self: self.env.ref('mx_pac_edi.payment_method_otros', raise_if_not_found=False))
    mx_pac_edi_payment_policy = fields.Selection(string='Payment Policy',
        selection=[('PPD', 'PPD'), ('PUE', 'PUE')],
        compute='_compute_mx_pac_edi_payment_policy')

    # -------------------------------------------------------------------------
    # HELPERS
    # -------------------------------------------------------------------------

    def _get_mx_pac_edi_signed_edi_document(self):
        self.ensure_one()

        cfdi_3_3_edi = self.env.ref('mx_pac_edi.edi_cfdi_3_3')
        return self.edi_document_ids.filtered(lambda document: document.edi_format_id == cfdi_3_3_edi and document.attachment_id)

    def _get_mx_pac_edi_issued_address(self):
        self.ensure_one()
        return self.company_id.partner_id.commercial_partner_id

    def _mx_pac_edi_decode_cfdi(self, cfdi_data=None):
        ...
        # Find a signed cfdi.
        if not cfdi_data:
            signed_edi = self._get_mx_pac_edi_signed_edi_document()
            if signed_edi:
    ...
    @api.model
    def _mx_pac_edi_cfdi_amount_to_text(self):
    ...
    @api.model
    def _mx_pac_edi_write_cfdi_origin(self, code, uuids):
        ''' Format the code and uuids passed as parameter in order to fill the mx_pac_edi_origin field.
        ...
        :return:        A valid string to be put inside the mx_pac_edi_origin field.
        '''
        return '%s|%s' % (code, ','.join(uuids))

    @api.model
    def _mx_pac_edi_read_cfdi_origin(self, cfdi_origin):
    ...
    @api.model
    def _mx_pac_edi_get_cfdi_partner_timezone(self, partner):
    ...
    def _compute_mx_pac_edi_cfdi_request(self):
        for move in self:
            if move.country_code != 'MX':
                move.mx_pac_edi_cfdi_request = False
            elif move.move_type == 'out_invoice':
                move.mx_pac_edi_cfdi_request = 'on_invoice'
            elif move.move_type == 'out_refund':
                move.mx_pac_edi_cfdi_request = 'on_refund'
            elif (move.payment_id and move.payment_id.payment_type == 'inbound' and
                'PPD' in move._get_reconciled_invoices().mapped('mx_pac_edi_payment_policy')):
                move.mx_pac_edi_cfdi_request = 'on_payment'
    ...
    def _compute_cfdi_values(self):
        ...
        '''
        for move in self:
            cfdi_infos = move._mx_pac_edi_decode_cfdi()

            move.mx_pac_edi_cfdi_uuid = cfdi_infos.get('uuid')
            move.mx_pac_edi_cfdi_supplier_rfc = cfdi_infos.get('supplier_rfc')
            move.mx_pac_edi_cfdi_customer_rfc = cfdi_infos.get('customer_rfc')
            move.mx_pac_edi_cfdi_amount = cfdi_infos.get('amount_total')

    @api.depends('move_type', 'invoice_date_due', 'invoice_date', 'invoice_payment_term_id', 'invoice_payment_term_id.line_ids')
    def _compute_mx_pac_edi_payment_policy(self):
        for move in self:
                ...
                if move.move_type == 'out_invoice':
                    ...
                    if move.invoice_date_due.month > move.invoice_date.month or \
                       ...
                       len(move.invoice_payment_term_id.line_ids) > 1:  # to be able to force PPD
                        move.mx_pac_edi_payment_policy = 'PPD'
                    else:
                        move.mx_pac_edi_payment_policy = 'PUE'
                else:
                    move.mx_pac_edi_payment_policy = 'PUE'
            else:
                move.mx_pac_edi_payment_policy = False

    # -------------------------------------------------------------------------
    # CONSTRAINTS
    # -------------------------------------------------------------------------

    @api.constrains('mx_pac_edi_origin')
    def _check_mx_pac_edi_origin(self):
        ...
        for move in self:
            if not move.mx_pac_edi_origin:
                continue

            # This method
            decoded_origin = move._mx_pac_edi_read_cfdi_origin(move.mx_pac_edi_origin)
            if not decoded_origin:
                raise ValidationError(error_message % move.mx_pac_edi_origin)
 ...
 xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/">
   ...
   <ns1:Body>
      <ns0:Consulta>
        ...
        for move in self:
            supplier_rfc = move.mx_pac_edi_cfdi_supplier_rfc
            customer_rfc = move.mx_pac_edi_cfdi_customer_rfc
            total = float_repr(move.mx_pac_edi_cfdi_amount, precision_digits=move.currency_id.decimal_places)
            ...
            if status == 'Vigente':
                move.mx_pac_edi_sat_status = 'valid'
            elif status == 'Cancelado':
                move.mx_pac_edi_sat_status = 'cancelled'
            elif status == 'No Encontrado':
                move.mx_pac_edi_sat_status = 'not_found'
            else:
                move.mx_pac_edi_sat_status = 'none'

    @api.model
    def _mx_pac_edi_cron_update_sat_status(self):
        ...
        # Update the 'mx_pac_edi_sat_status' field.
        cfdi_edi_format = self.env.ref('mx_pac_edi.edi_cfdi_3_3')
        to_process = self.env['account.edi.document'].search([
            ('edi_format_id', '=', cfdi_edi_format.id),
            ('state', 'in', ('sent', 'cancelled')),
            ('move_id.mx_pac_edi_sat_status', 'in', ('undefined', 'not_found', 'none')),
        ])
        to_process.move_id.mx_pac_edi_update_sat_status()

        # Handle the case when the invoice has been cancelled manually government-side.
        to_process\
            .filtered(lambda doc: doc.state == 'sent' and doc.move_id.mx_pac_edi_sat_status == 'cancelled')\
            .move_id\
            .button_cancel()
    ...
    def _update_payments_edi_documents(self):
        ...
        for payment in self:
            if payment.mx_pac_edi_cfdi_uuid:
                payment.mx_pac_edi_origin = payment._mx_pac_edi_write_cfdi_origin('04', [payment.mx_pac_edi_cfdi_uuid])
        return super()._update_payments_edi_documents()

    def _post(self, soft=True):
        # OVERRIDE
        certificate_date = self.env['mx_pac_edi.certificate'].sudo().get_mx_current_datetime()

        for move in self:

            issued_address = move._get_mx_pac_edi_issued_address()
            tz = self._mx_pac_edi_get_cfdi_partner_timezone(issued_address)
            tz_force = self.env['ir.config_parameter'].sudo().get_param('mx_pac_edi_tz_%s' % move.journal_id.id, default=None)
            if tz_force:
                tz = timezone(tz_force)

            move.mx_pac_edi_post_time = fields.Datetime.to_string(datetime.now(tz))

            if move.mx_pac_edi_cfdi_request in ('on_invoice', 'on_refund'):
    ...
    def button_draft(self):
        ...
        for move in self:
            if move.mx_pac_edi_cfdi_uuid:
                move.mx_pac_edi_origin = move._mx_pac_edi_write_cfdi_origin('04', [move.mx_pac_edi_cfdi_uuid])
            if move.payment_id:
                move.payment_id.mx_pac_edi_force_generate_cfdi = False
            elif move.statement_line_id:
                move.statement_line_id.mx_pac_edi_force_generate_cfdi = False
    ...
    def _reverse_moves(self, default_values_list, cancel=False):
        ...
        for default_vals, move in zip(default_values_list, self):
            if move.mx_pac_edi_cfdi_uuid:
                default_vals['mx_pac_edi_origin'] = move._mx_pac_edi_write_cfdi_origin('01', [move.mx_pac_edi_cfdi_uuid])
