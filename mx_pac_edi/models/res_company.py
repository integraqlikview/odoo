...
class ResCompany(models.Model):
    ...
    mx_pac_edi_certificate_ids = fields.Many2many('mx_pac_edi.certificate',
        string='Certificates')

    # == Address ==
    mx_pac_edi_colony_code = fields.Char(
        string='Colony Code',
        compute='_compute_mx_pac_edi_colony_code',
        inverse='_inverse_mx_pac_edi_colony_code',
    ...
    mx_pac_edi_colony = fields.Char(
        compute='_compute_mx_pac_edi_colony',
        inverse='_inverse_mx_pac_edi_colony')
    ...
    def _compute_mx_pac_edi_colony(self):
        for company in self:
            address_data = company.partner_id.sudo().address_get(adr_pref=['contact'])
            if address_data['contact']:
                partner = company.partner_id.sudo().browse(address_data['contact'])
                company.mx_pac_edi_colony = partner.mx_pac_edi_colony
            else:
                company.mx_pac_edi_colony = None

    def _inverse_mx_pac_edi_colony(self):
        for company in self:
            company.partner_id.mx_pac_edi_colony = company.mx_pac_edi_colony

    def _compute_mx_pac_edi_colony_code(self):
        for company in self:
            address_data = company.partner_id.sudo().address_get(
                adr_pref=['contact'])
            if address_data['contact']:
                partner = company.partner_id.browse(address_data['contact'])
                company.mx_pac_edi_colony_code = (
                    partner.mx_pac_edi_colony_code)

    def _inverse_mx_pac_edi_colony_code(self):
        for company in self:
            company.partner_id.mx_pac_edi_colony_code = (
                company.mx_pac_edi_colony_code)
    ...
    def _load_xsd_files(self, url):
        ...
        attachment = env.ref('mx_pac_edi.%s' % xsd_fname, False)
        ...
        self._cr.execute(
            ...
            VALUES (%s, %s, 'mx_pac_edi', 'ir.attachment', true)""", (
