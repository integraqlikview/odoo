...
class AccountEdiFormat(models.Model):
    ...
    @api.model
    def _mx_pac_edi_get_serie_and_folio(self, move):
    ...
    @api.model
    def _mx_pac_edi_cfdi_append_addenda(self, move, cfdi, addenda):
    ...
    @api.model
    def _mx_pac_edi_check_configuration(self, move):
        company = move.company_id
        pac_name = company.mx_pac_edi_pac

        errors = []

        # == Check the certificate ==
        certificate = company.mx_pac_edi_certificate_ids.sudo().get_valid_certificate()
        if not certificate:
            errors.append(_('No valid certificate found'))

        # == Check the credentials to call the PAC web-service ==
        if pac_name:
            pac_test_env = company.mx_pac_edi_pac_test_env
            pac_password = company.mx_pac_edi_pac_password
            if not pac_test_env and not pac_password:
                errors.append(_('No PAC credentials specified.'))
        else:
            errors.append(_('No PAC specified.'))

        # == Check the 'mx_pac_edi_decimal_places' field set on the currency  ==
        currency_precision = move.currency_id.mx_pac_edi_decimal_places
    ...
    @api.model
    def _mx_pac_edi_format_error_message(self, error_title, errors):
    ...
    def _mx_pac_edi_get_common_cfdi_values(self, move):
        ...
        if move.mx_pac_edi_origin:
            origin_type, origin_uuids = move._mx_pac_edi_read_cfdi_origin(move.mx_pac_edi_origin)
        else:
            origin_type = None
            origin_uuids = []

        return {
            **self._mx_pac_edi_get_serie_and_folio(move),
            'certificate': certificate,
            'certificate_number': certificate.serial_number,
            'certificate_key': certificate.sudo().get_data()[0],
            'record': move,
            'cfdi_date': move.mx_pac_edi_post_time.strftime('%Y-%m-%dT%H:%M:%S'),
            'supplier': supplier,
            'customer': customer,
            'customer_rfc': customer_rfc,
            'issued_address': move._get_mx_pac_edi_issued_address(),
    ...
    def _mx_pac_edi_get_invoice_line_cfdi_values(self, invoice, line):
    ...
    def _mx_pac_edi_get_invoice_cfdi_values(self, invoice):
        ''' Doesn't check if the config is correct so you need to call _mx_pac_edi_check_config first.
        ...
        cfdi_values = {
            **self._mx_pac_edi_get_common_cfdi_values(invoice),
            'document_type': 'I' if invoice.move_type == 'out_invoice' else 'E',
            'currency_name': invoice.currency_id.name,
            'payment_method_code': (invoice.mx_pac_edi_payment_method_id.code or '').replace('NA', '99'),
            'payment_policy': invoice.mx_pac_edi_payment_policy,
        }
        ...
        if cfdi_values['customer'].country_id.mx_pac_edi_code != 'MEX' and cfdi_values['customer_rfc'] not in ('XEXX010101000', 'XAXX010101000'):
            cfdi_values['customer_fiscal_residence'] = cfdi_values['customer'].country_id.mx_pac_edi_code
        else:
        ...
        for line in invoice_lines:
            cfdi_values['invoice_line_values'].append(self._mx_pac_edi_get_invoice_line_cfdi_values(invoice, line))
    ...
    def _mx_pac_edi_export_payment_cfdi(self, move):
        ...
        * error:        An error if the cfdi was not successfuly generated.
        ...
        for partial, amount, invoice_line in move._get_reconciled_invoices_partials():
            ...
            if not invoice.mx_pac_edi_cfdi_request:
                continue
            ...
            invoice_vals_list.append({
                ...
                'amount_paid': amount,
                **self._mx_pac_edi_get_serie_and_folio(invoice),
            })
        ...
        payment_method_code = move.mx_pac_edi_payment_method_id.code
        ...
        if partner_bank.country and partner_bank.country.code != 'MX':
            ...
            partner_bank_vat = partner_bank.mx_pac_edi_vat
        ...
        cfdi_values = {
            **self._mx_pac_edi_get_common_cfdi_values(move),
            ...
            'receiver_vat_ord': is_payment_code_receiver_ok and move.journal_id.bank_account_id.bank_id.mx_pac_edi_vat,
        ...
        if cfdi_values['customer'].country_id.mx_pac_edi_code != 'MEX':
            cfdi_values['customer_fiscal_residence'] = cfdi_values['customer'].country_id.mx_pac_edi_code
        else:
            cfdi_values['customer_fiscal_residence'] = None

        cfdi = self.env.ref('mx_pac_edi.payment10')._render(cfdi_values)

        decoded_cfdi_values = move._mx_pac_edi_decode_cfdi(cfdi_data=cfdi)
        cfdi_cadena_crypted = cfdi_values['certificate'].sudo().get_encrypted_cadena(decoded_cfdi_values['cadena'])
        decoded_cfdi_values['cfdi_node'].attrib['Sello'] = cfdi_cadena_crypted
    ...
    def _mx_pac_edi_get_finkok_credentials(self, move):
        if move.company_id.mx_pac_edi_pac_test_env:
            return {
            ...
            if not move.company_id.mx_pac_edi_pac_username or not move.company_id.mx_pac_edi_pac_password:
                return {
                    'errors': [_("The username and/or password are missing.")]
                }

            return {
                'username': move.company_id.mx_pac_edi_pac_username,
                'password': move.company_id.mx_pac_edi_pac_password,
    ...
    def _mx_pac_edi_finkok_sign(self, move, credentials, cfdi):
        try:
    ...
    def _mx_pac_edi_finkok_sign_invoice(self, invoice, credentials, cfdi):
        return self._mx_pac_edi_finkok_sign(invoice, credentials, cfdi)

    def _mx_pac_edi_finkok_cancel_invoice(self, invoice, credentials, cfdi):
        return self._mx_pac_edi_finkok_cancel(invoice, credentials, cfdi)

    def _mx_pac_edi_finkok_sign_payment(self, move, credentials, cfdi):
        return self._mx_pac_edi_finkok_sign(move, credentials, cfdi)

    def _mx_pac_edi_finkok_cancel_payment(self, move, credentials, cfdi):
        return self._mx_pac_edi_finkok_cancel(move, credentials, cfdi)

    def _mx_pac_edi_get_solfact_credentials(self, move):
        if move.company_id.mx_pac_edi_pac_test_env:
            return {
    ...
    def _mx_pac_edi_solfact_sign(self, move, credentials, cfdi):
        try:
            transport = Transport(timeout=20)
    ...
    def _mx_pac_edi_solfact_cancel(self, move, credentials, cfdi):
        uuids = [move.mx_pac_edi_cfdi_uuid]
        certificates = move.company_id.mx_pac_edi_certificate_ids
        certificate = certificates.sudo().get_valid_certificate()
        cer_pem = certificate.get_pem_cer(certificate.content)
        key_pem = certificate.get_pem_key(certificate.key, certificate.password)
    ...
    def _mx_pac_edi_solfact_sign_invoice(self, invoice, credentials, cfdi):
        return self._mx_pac_edi_solfact_sign(invoice, credentials, cfdi)

    def _mx_pac_edi_solfact_cancel_invoice(self, invoice, credentials, cfdi):
        return self._mx_pac_edi_solfact_cancel(invoice, credentials, cfdi)

    def _mx_pac_edi_solfact_sign_payment(self, move, credentials, cfdi):
        return self._mx_pac_edi_solfact_sign(move, credentials, cfdi)

    def _mx_pac_edi_solfact_cancel_payment(self, move, credentials, cfdi):
        return self._mx_pac_edi_solfact_cancel(move, credentials, cfdi)

    def _mx_pac_edi_get_sw_token(self, credentials):
    ...
    def _mx_pac_edi_get_sw_credentials(self, move):
        if move.company_id.mx_pac_edi_pac_test_env:
            credentials = {
                'username': 'demo',
                'password': '123456789',
        ...
        credentials.update(self._mx_pac_edi_get_sw_token(credentials))

        return credentials

    def _mx_pac_edi_sw_call(self, url, headers, payload=None):
        try:
    ...
    def _mx_pac_edi_sw_sign(self, move, credentials, cfdi):
...
""" % {'boundary': boundary, 'cfdi_b64': cfdi_b64}
        ...
        response_json = self._mx_pac_edi_sw_call(credentials['sign_url'], headers, payload=payload)
    ...
    def _mx_pac_edi_sw_cancel(self, move, credentials, cfdi):
        headers = {
            'Authorization': "bearer " + credentials['token'],
        ...
        cfdi_infos = move._mx_pac_edi_decode_cfdi(cfdi_data=cfdi)
        ...
        response_json = self._mx_pac_edi_sw_call(credentials['cancel_url'], headers, payload=payload.encode('UTF-8'))
    ...
    def _mx_pac_edi_sw_sign_invoice(self, invoice, credentials, cfdi):
        return self._mx_pac_edi_sw_sign(invoice, credentials, cfdi)

    def _mx_pac_edi_sw_cancel_invoice(self, invoice, credentials, cfdi):
        return self._mx_pac_edi_sw_cancel(invoice, credentials, cfdi)

    def _mx_pac_edi_sw_sign_payment(self, move, credentials, cfdi):
        return self._mx_pac_edi_sw_sign(move, credentials, cfdi)

    def _mx_pac_edi_sw_cancel_payment(self, move, credentials, cfdi):
        return self._mx_pac_edi_sw_cancel(move, credentials, cfdi)
    ...
    def _is_required_for_payment(self, move):
        ...
        if (move.payment_id or move.statement_line_id).mx_pac_edi_force_generate_cfdi:
            return True

        reconciled_invoices = move._get_reconciled_invoices()
        return 'PPD' in reconciled_invoices.mapped('mx_pac_edi_payment_policy')
    ...
    def _post_invoice_edi(self, invoices, test_mode=False):
        ...
        for invoice in invoices:
            ...
            errors = self._mx_pac_edi_check_configuration(invoice)
            if errors:
                edi_result[invoice] = {
                    'error': self._mx_pac_edi_format_error_message(_("Invalid configuration:"), errors),
                }
                continue

            # == Generate the CFDI ==
            res = self._mx_pac_edi_export_invoice_cfdi(invoice)
            if res.get('errors'):
                edi_result[invoice] = {
                    'error': self._mx_pac_edi_format_error_message(_("Failure during the generation of the CFDI:"), res['errors']),
                }
                continue
            ...
            if test_mode:
                ...
                pac_name = invoice.company_id.mx_pac_edi_pac

                credentials = getattr(self, '_mx_pac_edi_get_%s_credentials' % pac_name)(invoice)
                if credentials.get('errors'):
                    edi_result[invoice] = {
                        'error': self._mx_pac_edi_format_error_message(_("PAC authentification error:"), credentials['errors']),
                        'attachment': self._create_invoice_cfdi_attachment(invoice, base64.encodebytes(cfdi_str)),
                    }
                    continue

                res = getattr(self, '_mx_pac_edi_%s_sign_invoice' % pac_name)(invoice, credentials, res['cfdi_str'])
                if res.get('errors'):
                    edi_result[invoice] = {
                        'error': self._mx_pac_edi_format_error_message(_("PAC failed to sign the CFDI:"), res['errors']),
                        'attachment': self._create_invoice_cfdi_attachment(invoice, base64.encodebytes(cfdi_str)),
                    }
                    continue

            addenda = invoice.partner_id.mx_pac_edi_addenda or invoice.partner_id.commercial_partner_id.mx_pac_edi_addenda
            if addenda:
                ...
                res['cfdi_signed'] = self._mx_pac_edi_cfdi_append_addenda(invoice, res['cfdi_signed'], addenda)
    ...
    def _cancel_invoice_edi(self, invoices, test_mode=False):
        ...
        for invoice in invoices:
            ...
            errors = self._mx_pac_edi_check_configuration(invoice)
            if errors:
                edi_result[invoice] = {'error': self._mx_pac_edi_format_error_message(_("Invalid configuration:"), errors)}
                continue
            ...
            if test_mode:
                ...
                credentials = getattr(self, '_mx_pac_edi_get_%s_credentials' % pac_name)(invoice)
                if credentials.get('errors'):
                    edi_result[invoice] = {'error': self._mx_pac_edi_format_error_message(_("PAC authentification error:"), credentials['errors'])}
                    continue

                signed_edi = invoice._get_mx_pac_edi_signed_edi_document()
                if signed_edi:
                    cfdi_data = base64.decodebytes(signed_edi.attachment_id.with_context(bin_size=False).datas)
                res = getattr(self, '_mx_pac_edi_%s_cancel_invoice' % pac_name)(invoice, credentials, cfdi_data)
                if res.get('errors'):
                    edi_result[invoice] = {'error': self._mx_pac_edi_format_error_message(_("PAC failed to cancel the CFDI:"), res['errors'])}
                    continue
    ...
    def _post_payment_edi(self, payments, test_mode=False):
        ...
        for move in payments:
            ...
            # == Check the configuration ==
            errors = self._mx_pac_edi_check_configuration(move)
            if errors:
                edi_result[move] = {
                    'error': self._mx_pac_edi_format_error_message(_("Invalid configuration:"), errors),
                }
            ...
            # == Generate the CFDI ==
            res = self._mx_pac_edi_export_payment_cfdi(move)
            if res.get('errors'):
                edi_result[move] = {
                    'error': self._mx_pac_edi_format_error_message(_("Failure during the generation of the CFDI:"), res['errors']),
                }
            ...
            if test_mode:
                ...
                pac_name = move.company_id.mx_pac_edi_pac

                credentials = getattr(self, '_mx_pac_edi_get_%s_credentials' % pac_name)(move)
                if credentials.get('errors'):
                    edi_result[move] = {
                        'error': self._mx_pac_edi_format_error_message(_("PAC authentification error:"), credentials['errors']),
                ...
                res = getattr(self, '_mx_pac_edi_%s_sign_payment' % pac_name)(move, credentials, res['cfdi_str'])
                if res.get('errors'):
                    edi_result[move] = {
                        'error': self._mx_pac_edi_format_error_message(_("PAC failed to sign the CFDI:"), res['errors']),
                        'attachment': self._create_payment_cfdi_attachment(move, base64.encodebytes(cfdi_str)),
    ...
    def _cancel_payment_edi(self, moves, test_mode=False):
        ...
        for move in moves:
            ...
            errors = self._mx_pac_edi_check_configuration(move)
            if errors:
                edi_result[move] = {'error': self._mx_pac_edi_format_error_message(_("Invalid configuration:"), errors)}
                continue
            ...
            if test_mode:
                ...
                credentials = getattr(self, '_mx_pac_edi_get_%s_credentials' % pac_name)(move)
                if credentials.get('errors'):
                    edi_result[move] = {'error': self._mx_pac_edi_format_error_message(_("PAC authentification error:"), credentials['errors'])}
                    continue

                signed_edi = move._get_mx_pac_edi_signed_edi_document()
                if signed_edi:
                    cfdi_data = base64.decodebytes(signed_edi.attachment_id.with_context(bin_size=False).datas)
                res = getattr(self, '_mx_pac_edi_%s_cancel_payment' % pac_name)(move, credentials, cfdi_data)
                if res.get('errors'):
                    edi_result[move] = {'error': self._mx_pac_edi_format_error_message(_("PAC failed to cancel the CFDI:"), res['errors'])}
                    continue
